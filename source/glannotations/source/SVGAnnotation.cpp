...
glat::SVGAnnotation::SVGAnnotation(const globjects::ref_ptr<glat::AbstractState>& initialState, const glat::RendererFactory& factory /*= RendererFactory()*/)
: AbstractAnnotation(initialState){
	m_renderer = factory.createRenderer(*this);
}

glat::SVGAnnotation::SVGAnnotation(const globjects::ref_ptr<glat::AbstractState>& initialState, std::string svgPathString, unsigned width, unsigned height, const glat::RendererFactory& factory /*= RendererFactory()*/)
